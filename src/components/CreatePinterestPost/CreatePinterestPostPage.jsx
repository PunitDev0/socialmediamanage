...
export default function CreatePinterestPostPage() {
  const [selectedMedia, setSelectedMedia] = useState([]); // Array for Standard (1) or Idea Pins (multiple)
  ...
  const { control, handleSubmit, setValue, watch, formState: { errors } } = useForm({
    defaultValues: {
      postType: "standard", // "standard" or "idea"
      title: "",
      description: "",
      content: [], // Array for Idea Pin pages (text + media)
      link: "",
      board: "",
      date: null,
      time: "12:00",
  ...
  const handleFileUpload = (e) => {
    ...
    Promise.all(readers).then((results) => {
      ...
      if (postType === "idea") {
        setValue("content", [
          ...watch("content"),
          ...results.map(() => ({ text: "" })), // Add empty text for each new media
        ]);
      }
    });
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight">Create Pinterest Post</h1>
        <p className="text-muted-foreground">Create or schedule a Pinterest Standard or Idea Pin</p>
      </div>
