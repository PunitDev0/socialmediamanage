...
export default function InstagramHashtagManager({
  control,
  watch,
  setValue,
  enableAutoHashtags,
  ...
  const addHashtag = () => {
    if (newHashtag && !hashtags.includes(newHashtag) && hashtags.length < 30) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium">Hashtags ({hashtags.length}/30)</p>
        <TooltipProvider>
          <Tooltip>
      ...
      <div className="flex gap-2">
        ...
        />
        <Button type="button" onClick={addHashtag} size="sm" disabled={hashtags.length >= 30}>
